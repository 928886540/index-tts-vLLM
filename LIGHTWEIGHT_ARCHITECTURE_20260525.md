...
1. 给 `/voices` 做更友好的前端选择和刷新。
2. 给 `/cache` 做简单清理入口，不做重后台。
3. 用可选 `/profiles` 保存多套 TAVO 配置预设，SQLite 只存 profile，不存明文 LLM key。
4. 增加本地日志目录 `logs/`，只记录必要错误和请求摘要。
5. 评估 SQLite task queue 是否真的需要。
6. 如果长文本取消需求强，再接入取消标记或任务状态。
...
| 音频卡 UI 完整化 | Claude | `static/tavo.js` | 播放按钮、加载中、失败重试、缓存命中提示 |
