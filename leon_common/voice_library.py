...
def _default_voice_lib_dir() -> str:
    env_path = os.getenv("LEON_VOICE_LIB_DIR")
    if env_path:
        return env_path
    leon_root = os.getenv("LEON_ROOT")
    if leon_root:
        return str(Path(leon_root) / "prompts" / "library")
    return str(Path(__file__).resolve().parents[1] / "prompts" / "library")


VOICE_LIB_DIR = _default_voice_lib_dir()
