...
_TTS_QUEUE = TtsQueue(tts_stream_lock)
...
def _refresh_live_job_queue_metrics(job: "_LiveStreamingJob") -> None:
...
def _read_cache_metadata(key: str) -> dict:
    ...
    _wav_path, json_path = snapshot_cache.cache_paths(key)
    try:
        ...
        async with _tts_inference_slot("dialogue_live", cache_key=job.cache_key, metrics=job.metrics):
            ...
            for idx, seg in enumerate(segments):
                ...
                job.segments_meta.append({
                    ...
                    "emotion_mode": "qwen_emo" if use_qwen_emo_seg else "default",
                })
...
async def _run_model_warmup(voice: Optional[str] = None, text: Optional[str] = None, force: bool = False) -> dict:
    ...
    try:
        async with _tts_inference_slot("warmup"):
...
async def tts_stream_handle(req: dict):
    ...
    async def run_infer():
        try:
            async with _tts_inference_slot("single_stream"):
...
async def tts_cache_stream_handle(req: dict):
    ...
    async def run_infer():
        try:
            async with _tts_inference_slot("single_cache_stream", cache_key=cache_key):
                emo_text = req.get("emo_text")
                use_emo_text = bool(args.qwen_emo and emo_text)
                if emo_text == "auto":
                    emo_text = None
...
async def tts_dialogue_stream_handle(req: dict):
    ...
    async def run_infer():
        try:
            async with _tts_inference_slot("dialogue_stream"):
                for idx, seg in enumerate(segments):
                    role = _normalize_role(seg.get("role") or "")
                    text = (seg.get("text") or "").strip()
                    if not text:
...
async def tts_dialogue_cache_stream_handle(req: dict):
    ...
    async def run_infer():
        try:
            async with _tts_inference_slot("dialogue_cache_stream", cache_key=cache_key):
                for idx, seg in enumerate(segments):
                    role = _normalize_role(seg.get("role") or "")
                    text = (seg.get("text") or "").strip()
                    if not text:
...
async def cache_audio_by_key(key: str, format: Optional[str] = None):
    ...
    return cache_audio_get_response(key, format, snapshot_cache.get_cached_audio)
...
async def cache_audio_by_key_head(key: str, format: Optional[str] = None):
    ...
    return cache_audio_head_response(key, format, snapshot_cache.get_cached_audio)
...
async def server_log_tail(n: int = 100, since: float = 0.0, filter: Optional[str] = None):
    ...
    return server_log_tail_response(LOG_BUFFER, n=n, since=since, filter_text=filter, case_sensitive_filter=False)
...
async def tts_dialogue_stream_job_endpoint(request: TTS_Dialogue_Request):
    ...
    return dialogue_job_start_response(
        ...
        prepared,
        LIVE_JOBS,
        snapshot_cache.get_cached_audio,
        _LiveStreamingJob,
        _run_dialogue_inference_to_job,
...
async def tts_dialogue_stream_job_delete_endpoint(job_id: str, preserve_completed: bool = False):
    ...
    return dialogue_job_delete_response(
        ...
        LIVE_JOBS,
        snapshot_cache.get_cached_audio,
        snapshot_cache.delete_cache,
        _gc_live_job,
        suppress_delete_errors=False,
    )
