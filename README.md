...
先解压 `leon-common`，再按需要把 `leon-vllm` 或 `leon-fast6g` 解压到同一个目录。双击解压目录里的启动器：

```text
<解压目录>\LEON-Launcher-Tauri.exe
```
...
- `LEON-Launcher-Tauri.exe`：Windows 桌面启动器。
- `scripts/`：启动器调用的共享启动脚本。
- `static/`：Tavo 注入前端、runtime 分片和测试页面。
- `vllm/`：vLLM API 后端、模型和运行时，来自可选 `leon-vllm` 包。
- `fast6g/`：6 GB 显存友好 API 后端、模型和运行时，来自可选 `leon-fast6g` 包。
- `config/profiles/`：启动器调音台配置。
- `prompts/library/`：共享音色和声腔参考音频。
