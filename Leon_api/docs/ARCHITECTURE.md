...
This project is a local IndexTTS2 + vLLM + Tavo integration package. It is not a SaaS backend, and it is not designed around public multi-user tenancy.
...
- `indextts2_api.py`: FastAPI service, default port `9880`.
- `indextts/`: IndexTTS2 inference and support modules.
- `static/tavo.js`: single Tavo injected script and player UI.
- `prompts/library/`: voice/reference audio library.
- `outputs/cache/`: generated WAV and metadata snapshots.
- `Leon_api/dev_tools/`: local smoke tests, payloads, Playwright runner script, audio analysis utilities.
- `Leon_api/docs/`: active collaboration state.
- `Leon_api/handoff_docs/`: historical handoff material.
