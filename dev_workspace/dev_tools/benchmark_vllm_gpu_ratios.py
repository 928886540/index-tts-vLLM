...
def restart_vllm(ratio: float):
    ...
    try:
        ...
        try:
            ...
            pass
        elapsed = round(time.perf_counter() - started, 3)
        return {"elapsed_s": elapsed, "health": health, "wrapper_timeout": True, **api_processes(), "gpu": gpu_snapshot()}
    elapsed = round(time.perf_counter() - started, 3)
    ...
    return {"elapsed_s": elapsed, "health": health, **api_processes(), "gpu": gpu_snapshot()}
...
def main():
    ...
    for ratio in RATIOS:
        ...
        try:
            ...
            with out_jsonl.open("a", encoding="utf-8") as fp:
                fp.write(json.dumps(ratio_record, ensure_ascii=False) + "\n")
            continue
        records.append(ratio_record)
        with out_jsonl.open("a", encoding="utf-8") as fp:
            fp.write(json.dumps(ratio_record, ensure_ascii=False) + "\n")
        for run_idx in range(1, RUNS_PER_RATIO + 1):
            records.append(run_job(ratio, run_idx, segments, out_jsonl))
