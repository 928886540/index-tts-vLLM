...
这里的 `<lan-ip>` 换成运行 IndexTTS2 服务那台电脑的局域网 IP。
...
- 音频卡上的重新载入按钮只会在你点击时清掉当前卡片的本地 audio 对象并重新请求；缓存是否复用仍由后端快照规则决定。
...
```text
Mode: 浏览器直连第三方 API 或 本地服务代理 /parse_text
...
- 详细提示词模板见 `TAVO_LLM_PROMPT_20260525.md`。
