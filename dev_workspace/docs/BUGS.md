...
| BUG-048 | fixed in code, needs real Tavo validation | frontend LIVE playback output / API backend live buffer | Generated cache WAV is audible and `/pcm` chunks are non-silent. LIVE now polls same-key PCM, outputs through AudioWorklet/ScriptProcessor queue before falling back to BufferSource, primes native audio on user gesture, and can switch to same-key native live `<audio>` before cache落盘 if WebAudio output is blocked. `/pcm` done now only means the requested stream tail is drained. | Real Tavo/mobile: start LIVE, confirm `/pcm` logs show queued output, audio is audible before cache落盘, and switching chat/app only pauses or falls back without losing the key. |
| BUG-049 | fixed in code, needs real Tavo validation | frontend status / lyric UI | Avatar-side status is reserved for the configured/current voice label only. LLM/TTS/LIVE progress moved into a one-line sticky toolbar inside the lyric panel, beside delete and page counter; raw backend text like "文本已拆分" is translated; spinner transform origin is fixed. | Real Tavo/mobile: generate AI LIVE, scroll lyrics, and confirm avatar-side line never shows progress/error copy while the lyric toolbar remains readable and fixed. |
...
- BUG-049 guard: `setStatus()` writes transient text to the one-line lyric toolbar progress element, then forces the avatar-side line back to the stable voice label or "音色未设置"; pure voice-label updates do not fill the progress element.
