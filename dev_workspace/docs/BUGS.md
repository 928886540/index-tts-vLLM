...
| BUG-053 | fixed in code, needs real Tavo validation | frontend / Tavo storage / LIVE lifecycle | LIVE jobs now persist a visible pending card immediately after `cacheKey` creation under both message-id and message-text hash keys. Script refresh or unstable message ids can restore the same key from the lazy ordinary card and keep cancel/reconnect available. Explicit LIVE exit/delete is the cleanup path. | Real Tavo/mobile: start LIVE, refresh/re-enter/lock screen before cache lands, confirm the lazy card says `流式生成中 · 点开继续`, opens the same key without a new job, and explicit exit/delete clears it. |
...
- BUG-053 v49 fix: pending storage is duplicated to `indextts_pending_jobs_text_<正文hash>`, and the lazy loader reads both message-id and text-hash pending keys, including the hash from `tavo.message.current().content`. This keeps the ordinary lazy card available after script refresh even if the message id changed or only the text hash matches.
- BUG-053 guard: Playwright starts a LIVE job, verifies chat-scoped pending storage before close, verifies the text-hash pending key, clears the message-id pending key, remounts the test page and confirms the same key is restored without a new POST, then confirms explicit LIVE exit clears both pending keys and sends DELETE. A separate smoke confirms empty lazy play generates the current bubble.
