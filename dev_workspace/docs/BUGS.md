...
| BUG-055 | fixed in code, needs real Tavo validation | frontend LIVE playback / mobile WebView audio session | `visibilitychange` / `pagehide` no longer pauses or suspends any LIVE track. Native MP3, segment audio, and explicit WebAudio diagnostics keep their current output/job; only the user pressing pause writes `pausedByUser`. | Real Tavo/mobile: start LIVE, switch app/open console/lock screen, confirm playback continues when the host allows audio, no "暂挂/点播放继续" copy appears, the same key/job stays alive, and only tapping pause stops playback. |
...
- BUG-055 root cause: frontend previously treated page/background lifecycle as an audio failure and entered a resumable pause/suspend state, which made native `<audio>` background playback look stopped and polluted track state with `pausedByUser` / `livePageSuspended`.
- BUG-055 fix: `visibilitychange` / `pagehide` now only record the page-hidden timestamp for diagnostics. They do not pause audio, stop WebAudio polling, clear streams, delete jobs, or write user-pause state. Automatic stream recoverable states no longer set `pausedByUser`; only the play/pause button does.
- BUG-055 guard: Playwright simulates hidden/visible transitions during default MP3 LIVE and explicit WebAudio LIVE, and asserts playback remains `playing`, no suspend copy appears, no DELETE/new POST occurs, `pausedByUser` / `livePageSuspended` stay false, and foreground return does not require a resume click or `start_s` reconnect.
