...
- AI role mapping must not include normal-only `对白` / `对话` / `台词` / `dialogue` rows by default. AI generation must not submit `voices.default`; every AI role needs an explicit mapping. Normal mode always submits `voices.default` and `voices.旁白` when narrator is configured; blank `对白` must be omitted so it inherits narrator/default. Explicit `对白` must submit all dialogue aliases with the user-selected voice.
...
- with zero playable tracks, the main play button is disabled and the music-note generate button stays enabled;
...
- restored LIVE pending jobs stored through `tavo.set` must be restored on full-player mount; clicking play must GET `/tts_dialogue_stream_job/<same_cache_key>?start_s=<last_second>` with no generation `POST`;
...
Tavo storage is authoritative. Runtime config, character voice config, saved tracks, and pending jobs should read `tavo.get` before local fallback and write through `tavo.set`; `localStorage` is only a compatibility fallback. A failed `tavo.set` must surface as a settings/save error instead of a false success.
...
15. Pending LIVE jobs restored from `tavo.get` or local fallback must keep `playbackMode=live`, `state=live`, and the last resume seconds (`lastWebAudioSec` / `lastElementSec` / `lastStalledSec`). Full-player mount should restore these cards before the play button remains disabled. D-mode pending jobs remain background/pending.
...
- In AI mode, the frontend must not submit a `default` voice fallback and must not display `cfg.defaultVoice` as the avatar-side current voice when an explicit role mapping is required.
