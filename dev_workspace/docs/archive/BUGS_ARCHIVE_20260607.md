...
## BUG-046: Tavo play button reuses deleted live cache key and LIVE buffering prompts are too noisy

Status: fixed in code, needs real Tavo validation

Reported: 2026-06-07

Repro: In Tavo LIVE mode, generate audio, delete the current card/audio, then press the main play button when no audio remains. The frontend can reconnect to the previous `cache_key`. During LIVE WebAudio stalls, the UI also shows proactive "跟不上/手动续播" style notices.

Evidence: User screenshot shows repeated WebAudio reconnects on the same `cacheKey=8dc36f...d57fa`, followed by `LIVE 连接转为可手动续播 reason=slow stream`. User reported that after deleting and pressing play, there was no current audio but the frontend connected to the old key. User requested simpler semantics: play only plays/pauses existing audio; new audio must be generated by the music-note button; when there is no audio, the play button should be disabled.

Hypothesis: The play click still falls into `generate(false)`, which can select pending/live tracks or legacy audio state and reconnect a stream. Delete/empty states do not clear all stream/key state strongly enough, and the play button is not disabled for empty history. WebAudio recovery treats slow/buffering as a user-facing state transition instead of silently buffering or letting the user pause/retry.

Root cause: The frontend still let the main play action enter the old generation path (`generate(false)`), so an empty/deleted state could fall through into stale audio or pending/live stream recovery. Delete paths also did not clear every playback owner (`currentCacheKey`, element audio source/dataset, WebAudio state, pending storage) before returning to the empty UI. LIVE recovery surfaced transport-health debug states as user-facing "跟不上/手动续播" copy, even when the better behavior is to pause/idle and let the user retry the same stream.

Fix: `static/tavo.runtime.parts/60_generate_flow.js`, `62_events_boot.js`, and `52_subtitle_media.js` split playback from generation: the main play button now only plays/pauses the current track, and new audio is created only by the music-note button. `44_element_audio.js` disables play when there is no playable current track. `48_track_history.js` clears stale cache/audio state and persists deletion through `tavo.set`, and full-player mount now restores saved/pending tracks before leaving the button disabled. `46_track_state.js` keeps same-key LIVE retry behavior but removes user-facing "实时生成跟不上/手动续播" notices, returns first-audio timeout to an idle retry state after 30s, and keeps background cache polling alive.

Guard: The main play button must never create a new generation job and must never resurrect a deleted `cache_key`. With zero current playable tracks it is disabled and the music-note button remains enabled. New audio is created only by the music-note generate button. Deleting the final card clears `currentCacheKey`, element audio dataset/source, WebAudio controller, pending state, and persisted history. Restored LIVE pending tracks must come back from `tavo.get` with `playbackMode=live`, `state=live`, and resume seconds, then reconnect the same `cache_key` with `start_s` and no POST. LIVE buffering may update compact state/logs, but must not show "实时生成跟不上", "手动续播", or forced fallback notices unless the user explicitly acts or the job reaches a terminal state.

## BUG-047: Tavo voice mapping falls back to default voice instead of surfacing missing role mapping

Status: fixed in code, needs real Tavo validation

Reported: 2026-06-07

Repro: In AI/multi-role mode, leave a role without explicit voice mapping and generate. The frontend can submit a `default` voice fallback, so the API backend resolves missing roles through the default voice instead of reporting the mapping problem in the lyric/player UI.

Evidence: User explicitly requested "不要给默认音色映射！没有映射就歌词页报错！" User also reported the avatar-side status showed `宝岛甜心` after generating instead of the currently configured voice, which is consistent with stale/default voice fallback being used in status/voice map resolution.

Hypothesis: `rolesListToVoicesMap()` always includes `default`, and `voiceNameForRole()` falls back to `voices.default` / `cfg.defaultVoice`. That can mask unmapped roles and show stale default voices in the header.

Root cause: AI mode reused normal/default mapping rules. `rolesListToVoicesMap()` inserted `default`, `voiceNameForRole()` fell back to `voices.default` / `cfg.defaultVoice`, and generated/restored AI tracks stored `cfg.defaultVoice` as the representative voice. That masked missing role mappings and could show a stale/default voice beside the avatar instead of the explicitly configured current role voice.

Fix: `static/tavo.runtime.parts/05_style_config.js` no longer adds `default` for AI voice maps. `42_playback_header.js` resolves AI roles only from explicit role mappings and chooses a representative explicit AI voice for the header. `50_settings_fields.js` requires explicit mappings for `旁白`, `用户`, the current Tavo character, and custom AI roles before generation. `48_track_history.js` and `60_generate_flow.js` store representative voices from the submitted role map instead of blindly using `cfg.defaultVoice`.

Guard: AI/multi-role generation should submit only explicitly configured role voices. If a role appears in segments/status without a mapping, the lyric/player should show a clear missing-mapping error instead of choosing `default`. Normal mode may still require `旁白` and optional `对白`, but AI mode should not silently inherit a default voice for unmapped roles.
...
Evidence: User report from 2026-06-06, reinforced on 2026-06-07: "你的设置页没保存到tavo set？！" Code evidence: `loadCharacterCfg()` reads `localStorage` before `tavo.get(...)`, so a stale local snapshot can override Tavo persistent storage on remount/re-entry. `saveConfig()` and picker quick-save swallow `tavo.set` failures and still allow the UI to show "设置已保存".

Hypothesis: confirmed.

Root cause: The Tavo frontend treated `localStorage` as authoritative in several load paths before durable Tavo storage. On WebView remount/re-entry, a stale local snapshot could override the real `tavo.get` value. Save paths also swallowed `tavo.set` failures, so the UI could show "设置已保存" even though Tavo persistent storage was not updated.

Fix: `static/tavo.runtime.parts/00_base_context.js` and `05_style_config.js` now load config/character config from `tavo.get` first and use `localStorage` only as fallback. `saveConfig()` / `saveCharacterCfg()` propagate `tavo.set` errors instead of swallowing them. `54_voice_picker.js` and `62_events_boot.js` catch those errors and show "设置保存失败" rather than a false success. Track history and pending jobs also prefer `tavo.get` before local fallback so deletion/re-entry cannot revive stale local keys.

Guard: Verify on `fast6g` and `vllm` in real Tavo that changing settings, saving, closing/reopening the panel, remounting the message, and re-entering the chat all show the same saved values. The success toast must only appear after `tavo.set` completes and the in-memory config is updated. If `tavo.set` throws, the UI must show "设置保存失败" and not silently fall back to pretending localStorage is enough.
