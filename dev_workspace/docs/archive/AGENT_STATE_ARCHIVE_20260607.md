...
## Latest Fix Snapshot: Tavo AI/LIVE Storage and Playback v16

Updated: 2026-06-07

Fix now in code:

- `static/tavo.js`, `static/tavo.runtime.js`, `static/tavo.runtime.manifest.json`, and `README.md` are bumped to `20260607-ai-live-v16`.
- Tavo settings/config now read durable `tavo.get` first and use `localStorage` only as fallback. `tavo.set` failures are surfaced as "设置保存失败" instead of false success.
- Track history and pending LIVE/D jobs also prefer `tavo.get`, so deletion/re-entry should not revive stale local `cache_key` data.
- The main play button now only plays/pauses the current track. New audio must be created by the music-note button. With zero playable tracks, play is disabled and the music-note button remains enabled.
- Full-player mount restores saved and pending/live tracks before leaving the player in an empty disabled state. Restored LIVE pending jobs keep `state=live`, resume seconds, and reconnect the same key with `start_s` and no POST.
- AI mode no longer submits `voices.default`. It requires explicit mappings for `旁白`, `用户`, current Tavo character, and custom AI roles; missing mappings surface as a frontend error.
- AI/LIVE job status copy now says concrete phases such as `AI+LIVE LLM 调用中，分析角色和情绪`, `AI+LIVE 分段完成，等待首段音频`, and `AI+LIVE 合成第 x/y 段中` instead of exposing raw `文本已拆分`.
- LIVE first-audio timeout returns to an idle/retry state after about 30s and keeps background cache polling; noisy user-facing "实时生成跟不上/手动续播" prompts are suppressed.
...
Still requires real Tavo/mobile validation: settings save/reopen through real `tavo.set`, AI missing-role error UI, LIVE first-audio behavior on the phone, and restored pending LIVE resume after app re-entry.
