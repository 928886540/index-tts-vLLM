...
def _segment_cache_payload(seg: dict) -> dict:
    ...
    if args.qwen_emo:
        ...
        item["emo_vec"] = _effective_segment_emo_vec_for_cache(seg)
        item["emo_text"] = seg.get("emo_text") or None
...
async def test_generate_endpoint(request: Request):
    ...
    try:
        ...
        generate_audio(
            ...
            emo_vector=None if direct_style_ref else style_entry.get("emo_vec", [0, 0, 0, 0, 0, 0, 0, 0.85]),
        ...
        async with _tts_inference_slot("dialogue_live", cache_key=job.cache_key, metrics=job.metrics):
            ...
            for idx, seg in enumerate(segments):
                ...
                if profile_emo_vec is not None:
                    emo_vec = _coerce_dialogue_emo_vec(profile_emo_vec)
                    emo_vec_source = "style_profile"
                else:
